...
module.exports = {
  name: "radio",
  description: "Spielt Radio",
  cooldown: 3,
  edesc: `Benutze diesen Command um im Radio mitzuhören\nBenutzung: ${PREFIX}radio <1-34>`,
  async execute(message, args, client) {
    //define the No args Embed, lmao
    let resultsEmbed = new Discord.MessageEmbed()
      .setTitle(`**Mögliche Radios**`)//
      .addFields(
        {
...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${Radiostations[34 - 1].split(" ")[1]})`, inline: true
      ...
      .setColor("#4B180F")
      .setFooter(`Schreibe: ${PREFIX}radio <1-34>`, client.user.displayAvatarURL())
      .setTimestamp();
    ...
    const song = {
      title: args2[0],
      url: args2[1],
      thumbnail: "https://mfp-network.com/wp-content/uploads/2022/07/Braun-orange-weisses-Logo-mit-Illustration-Festival-Pandaemonium.png",
      duration: 10000,
    };
    let a, b;
    if (!serverQueue) {
    ...
    if (!serverQueue)
      message.channel.send(new MessageEmbed().setColor("#4B180F")
        .setDescription(`**👍 Beigetreten \`${channel.name}\` 📄 bouned \`#${message.channel.name}\`**`)
        .setFooter(`${message.author.username}#${message.author.discriminator}`));
    //send Search something embed
    message.channel.send(new MessageEmbed().setColor("#4B180F")
      .setDescription(`**:radio: Suche 🔍 \`${Radiostations[i - 1].split(" ")[0]}\`**`));
    //mute yourself
    ...
    */
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .setURL(song.url)
        .setDescription(`\`\`\`Wurde zum Warten verurteilt.\`\`\``)
        .addField("Vermutliche Zeit bis es abgehen kann:", `\`${estimatedtime}\``, true)
        .addField("Position in Warteschlange", `**\`${serverQueue.songs.length - 1}\`**`, true)
        .setFooter(`Angefordert von: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
    ...
    try {
      ...
      await channel.leave();
      return message.channel.send(`Konnte keinem VC beitreten: ${error}`).catch(console.error);
    }
    //sende bestätigung
  }
};
