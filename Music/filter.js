...
module.exports = {
  name: "filter",
  description: "Audio - Effekte einstellen",
  aliases: ["f"],
  cooldown: 3,
  edesc: `Geben Sie diesen Befehl ein, um den aktuellen Audioeffekt zu ändern - art \nBenutzung: ${PREFIX}filter <Filtertype>`,
  ...
  async execute(message, args, client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message, "Es ist eine Suche im Gange!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message, "Bitte treten Sie zuerst einem Voice Channel bei")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("#4B180F")
        .setTitle("Verwenden Sie einen der folgenden:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`chorus\`
        \`karaoke\`
        \`sofa (macht Audio für Kopfhörer/Headset geeignet)\`
        \`desilencer (entfernt automatisch die Stille im Lied)\`
        \`clear\`   ---  Alle Filter werden entfernt`)
        .setFooter(`Beispiel: ${PREFIX}filter bassboost`)
        )
        break;
    }
    //set choice to zero
    ...
    try {
      ...
      message.channel.send(new MessageEmbed()
      .setColor("#4B180F")
      .setAuthor("Anwendung: " + args[0], "https://mfp-network.com/wp-content/uploads/2022/07/Braun-orange-weisses-Logo-mit-Illustration-Festival-Pandaemonium.png",
      "https://mfp-network.com/ghostmusic")).then(msg =>{
